...
public class GraphComponent extends JComponent {
	...
	private void drawMap(Graphics g) {
		...
		for (Node j : _graph.getNodes()) {
			...
			g.fillOval(p.cX - _nodeRadius / 2, p.cY - _nodeRadius / 2, _nodeRadius,
					_nodeRadius);
	...
	private void drawCircleOnALine(Graphics g, int x1, int y1, int x2, int y2,
			int virtualLength, int virtualLocation, int diam, Color c, String txt) {

		// The actual length of the line
		double lineActualLength = Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2))
				- 45;

		// the angle of the line with the horizontal axis
		double alpha = Math
				.atan(((double) Math.abs(x1 - x2)) / ((double) Math.abs(y1 - y2)));
		...
		// draw the point
		g.setColor(c);
		g.drawOval(x1 + xDir * ((int) x) - diam / 2, y1 + yDir * ((int) y) - diam / 2,
				diam, diam);
